...
#         img = np.array(img)
#     noise = np.random.normal(size=img.shape[:2]).astype("float32")
#     current_snr = np.mean(img) / np.std(noise)
#     # current_snr_db = 10*np.log10(current_snr)
#     snr = 10**(snr_db/10.0)
...
#     img = img + noise
#     # if save:
#     #     cv2.imwrite('/home/XXX/图片/rolled-in_scale_184_'+str(snr_db)+'.jpg',img[:,:,0])
#     return img
