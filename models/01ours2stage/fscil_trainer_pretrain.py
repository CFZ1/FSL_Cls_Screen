...
class FSCILTrainer(Trainer):
    ...
    def get_dataloader(self, session, base_meta = False):
        ...
        if self.args.procedure in ['multiLabel','singleLabel1']:
            if self.args.trainMode in [1,2]:
                trainset, trainloader, valloader, testloader= self.args.Dataset.get_dataloader_PretrainONE(self.args)
                self.args.not_data_init = True
                self.trlog['max_acc'][session] = -10000
            elif self.args.trainMode in [3,4]:
                trainset, trainloader, valloader, testloader = self.args.Dataset.get_dataloader_PretrainTWO(self.args)
                self.args.not_data_init = True
            else:
                print('not implement')
    ...
    def base_train(self,model, trainloader, optimizer, scheduler, epoch, args, other_args = None):
        ...
        if self.args.trainMode in [1,2]:
            for i, batch in enumerate(trainloader, 1):
                ...
                mask_sigmoid = model(data)
                loss_mask = F.binary_cross_entropy(mask_sigmoid,torch.zeros_like(mask_sigmoid).detach())
                optimizer.zero_grad()
                loss_mask.backward()
                optimizer.step()
                tl.add(loss_mask.item(),mask_sigmoid.size(0))
            tl = tl.item()
            print('--------epo {}, train,loss_mask={:.4f} '.format(epoch,tl))
        return tl,0.0       
        ...
        if self.args.trainMode in [1,2]:
            ...
            with torch.no_grad():
                for i, batch in enumerate(testloader, 1):
                    ...
                    mask_sigmoid = model(data)
                    loss_mask = F.binary_cross_entropy(mask_sigmoid,torch.zeros_like(mask_sigmoid).detach())
                    tl1.add(loss_mask.item(),mask_sigmoid.size(0))
                tl1 = tl1.item()
                print('val---epo {},loss_mask={:.4f} '.format(epoch,tl1))    
            return 0.0,0.0,[-tl1]
