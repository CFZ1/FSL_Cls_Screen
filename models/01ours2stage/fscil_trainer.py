...
class FSCILTrainer(Trainer):
    def __init__(self, args):
        ...
        print('hello')
        for param_key in self.model.state_dict().keys():
            if param_key not in self.best_model_dict.keys():
                self.best_model_dict[param_key] = self.model.state_dict()[param_key]
    ...
    def test(self,model, testloader, epoch,args, session,validation=True, mode_test='test',result_list=None):
        ...
        with torch.no_grad():
            ...
            if validation is not True:
                ...
                # if session == 1:
                #     name=['img_name','labels','predict']+['logits']*lgt.shape[-1]
                #     list_save = []
                #     list_save.append(testloader.dataset.data)
                #     list_save.append(np.int64(lbs.numpy()).tolist())
                #     pred = np.argmax(lgt.numpy(), axis=1)
                #     list_save.append(pred.tolist())
                #     for i in range(lgt.shape[-1]):
                #         list_save.append(lgt[:,i].numpy().tolist())
                #     ar2 = list(map(list,zip(*list_save))) #行列交换
                #     test=pd.DataFrame(columns=name,data=ar2)
                #     save_csv_pth = os.path.join(args.save_path, 'session' + str(session) + 'result.csv')
                #     test.to_csv(save_csv_pth)
                if args.procedure in ['singleLabel1']:
                    #---print---
                    results_print = {}
                    if session==0:
