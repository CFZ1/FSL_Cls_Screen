...
class MYNET(nn.Module):
    ...
    def __init__(self, args, mode=None):
        ...
        '''
        args.trainMode
        trainMode=1: 1）mask损失；2）多分类(冻结backbone和mask)
        trainMode=2: 1）mask损失；2）多分类(冻结mask)
        trainMode=3: 1）mask损失+二分类损失；2）多分类(冻结backbone和mask)
        trainMode=4: 1）mask损失+二分类损失；2）多分类(冻结mask)
        args.trainModeStage: 1/2
        '''
        ...
        if args.trainMode in [3,4]:  
            self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
            self.fcBin = nn.Linear(self.num_features, 2, bias=False)
        if args.trainModeStage in [2]:
            #冻结mask
            self.conv_mask.eval()
            for param in self.conv_mask.parameters():
                param.requires_grad = False 
            #冻结mask
#             if args.trainMode in [1,3]: #冻结backbone
#                 self._freeze_stages(2)
    ...
    def _freeze_stages(self,frozen_stages):
        if frozen_stages >= 0:
            ...
            for m in [self.encoder.conv1, self.encoder.bn1]:
                for param in m.parameters():
                    param.requires_grad = False
        for i in range(1, frozen_stages + 1):
            m = getattr(self.encoder, f'layer{i}')
            m.eval()
            for param in m.parameters():
                param.requires_grad = False     
    ...
    def forward(self, input):
        if self.args.trainModeStage in [1]: 
            if self.args.trainMode in [1,2]:
                x = self.encoder(input)
                if self.args.debug3_maskLoss:
                    mask_sigmoid = torch.sigmoid(self.conv_mask(x))
            return mask_sigmoid     
