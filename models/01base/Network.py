...
class MYNET(nn.Module):
    ...
    def forward_metric(self, x):
        x,mask_sigmoid = self.encode(x) #-----------------------debug3_maskLoss_4
        if 'cos' in self.mode:
            ...
            x = self.args.temperature * x
        return x,mask_sigmoid #-----------------------debug3_maskLoss_5
    ...
    def forward(self, input):
        if self.mode != 'encoder':
            input,mask_sigmoid = self.forward_metric(input)#-----------------------debug3_maskLoss_6
            return input,mask_sigmoid #-----------------------debug3_maskLoss_7
        elif self.mode == 'encoder':
            input,_ = self.encode(input)#-----------------------debug3_maskLoss_15
            return input
        else:
    ...
    def update_fc(self,dataloader,class_list,session):
        for batch in dataloader:
            ...
            data=self.encode(data)[0].detach()#-----------------------debug3_maskLoss_16e
