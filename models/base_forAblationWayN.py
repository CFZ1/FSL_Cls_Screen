...
class Trainer(object, metaclass=abc.ABCMeta):
    ...
    def replace_base_fc(self,trainset, transform, model, args):
        ...
        return model
    def cons_mobilescreen_openness(self, train_set, run_path=None):  ##230415
        train_idx_list = []
        if run_path==None:
            ...
            for idx,target_id in enumerate(train_set.data):
                if target_id.split(train_set.img_path+'/')[-1] in run_trainData:
                    for novel_cls_i in train_set.novel_cls:
                        if novel_cls_i in target_id.split(train_set.img_path+'/')[-1]:
                            train_idx_list.append(idx)
        print(' train_idx_list: ',len(train_idx_list))          
        test_idx_list = [i for i in list(range(0,train_set.origialLegth)) if i not in train_idx_list] ###diff
    ...
    def train_s1_manyRuns(self, result_list):
        ...
        if self.args.procedure in ['multiLabel','singleLabel1']:
            if ('mobilescreen' in self.args.dataset) or ('Magnetic' in self.args.dataset):
                ...
                print('self.model.fc',self.model.module.fc.weight.shape)
...
#                     run_path = os.path.join(self.args.dataroot,'openness_split_5w5s/r'+str(run)+'.txt')##230415
                    run_path = os.path.join(self.args.dataroot,'openness_split_3w1s/r'+str(run)+'.txt')##230415
                    if not os.path.exists(run_path):
                        run_path = None                  
                train_set_new, trainloader,testloader = self.cons_mobilescreen_openness(train_set,run_path) ##230415
            elif 'NEU' in self.args.dataset:
...
#                 path = os.path.join(self.args.dataroot,'split_'+str(self.args.way)+'w'+str(self.args.shot)+'s_save/r'+str(run)+'n'+str(np.mean(ta_cls_perCls[self.args.base_class:self.args.num_classes])*1000).split('.')[0]+'a'+str(np.mean(ta_cls_perCls)*1000).split('.')[0]+'.txt')
                path = os.path.join(self.args.dataroot,'openness_split_'+str(self.args.way)+'w'+str(self.args.shot)+'s_save/r'+str(run)+'.txt')##230415
                str1 = '\n'
                f=open(path,"w")
