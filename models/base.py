...
#         test_set.add_noise_ONoff = True
    ...
    def cons_NEU(self, train_set, run_path=None):
        ...
        if run_path==None:
            ...
            print('data_path')
            run_trainData = [x.strip() for x in open(run_path).readlines()]
            for idx,target_id in enumerate(train_set_new.data):
                if target_id.split(train_set_new.img_path+'/')[-1] in run_trainData:
                    train_idx_list.append(idx)
    ...
    def train_s1_manyRuns(self, result_list):
        ...
        for run in range(self.args.test_runs):
            ...
            if self.args.test_random:
                ...
                run_path = os.path.join(self.args.dataroot,'split_'+str(self.args.way)+'w'+str(self.args.shot)+'s/r'+str(run)+'.txt')
                if not os.path.exists(run_path):
                    run_path = None 
            if ('mobilescreen' in self.args.dataset) or ('Magnetic' in self.args.dataset):                 
                train_set_new, trainloader,testloader = self.cons_mobilescreen(train_set,run_path)
            elif 'NEU' in self.args.dataset:
                train_set_new, trainloader,testloader = self.cons_NEU(train_set,run_path)  
            #---------------------创建训练集和测试集------------------
            #---------------------2.2:trainloader更新模型
...
#             result_list.append(train_set_new.data) #------------230402
            ...
            #------------save_data_1s
            save_data_flag = ('mobilescreen' in self.args.dataset) or (self.args.dataset in ['NEU','Magnetic'])
            if save_data_flag and (self.args.save_data):
                data_save = [train_set_new.data[i].split(train_set_new.img_path+'/')[-1] for i in range(len(train_set_new))] 
                path = os.path.join(self.args.dataroot,'split_'+str(self.args.way)+'w'+str(self.args.shot)+'s_save/r'+str(run)+'.txt')
                str1 = '\n'
                f=open(path,"w")
